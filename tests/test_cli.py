...
def test_doctor_all_good(monkeypatch):
    ...
    def fake_get(url, *a, **k):
        if url.endswith('/api/version'):
            return FakeResponse({'version': '0.6.0'})
        return FakeResponse({'models': [{'name': 'nomic-embed-text:latest'},
                                        {'name': 'llama3.2:latest'}]})

    monkeypatch.setattr('requests.get', fake_get)
    # The embedding round-trip probe
    monkeypatch.setattr('requests.post',
                        lambda *a, **k: FakeResponse({'embedding': [0.1, 0.2]}))
    result = CliRunner().invoke(cli, ['doctor'])
    assert result.exit_code == 0, result.output
    assert 'Embedding round-trip works' in result.output
    assert 'Taxonomy valid' not in result.output  # conftest points elsewhere
    assert 'All good!' in result.output


def test_doctor_fails_on_broken_taxonomy(tmp_path, monkeypatch):
    """A trailing comma in taxonomy.json silently disables the whole
    category funnel — doctor must FAIL with the parse location, not shrug."""
    bad = tmp_path / 'taxonomy.json'
    bad.write_text('{"version": 1, "buckets": [,]}', encoding='utf-8')
    monkeypatch.setenv('TAXONOMY_PATH', str(bad))
    ...
    assert 'JSON error at line' in result.output
    assert 'DISABLED' in result.output


def test_doctor_fails_on_wrong_shape_without_crashing(tmp_path, monkeypatch):
    """Parseable-but-wrong shapes (string buckets, top-level lists) must
    FAIL with a pointer — doctor crashing on the file it exists to
    diagnose is the worst possible outcome."""
    bad_tax = tmp_path / 'taxonomy.json'
    bad_tax.write_text('{"version": 1, "buckets": ["a", "b"]}',
                       encoding='utf-8')
    monkeypatch.setenv('TAXONOMY_PATH', str(bad_tax))
    bad_seeds = tmp_path / 'seeds.json'
    bad_seeds.write_text('{"not": "a list"}', encoding='utf-8')
    monkeypatch.setenv('SEED_TOPICS_PATH', str(bad_seeds))
    ...
    monkeypatch.setattr('requests.get', refuse)
    result = CliRunner().invoke(cli, ['doctor'])
    # A clean exit(1), never an AttributeError/TypeError traceback
    assert isinstance(result.exception, SystemExit)
    assert result.exit_code == 1
    assert "every bucket needs 'id', 'name', and 'anchor'" in result.output
    assert 'the file must be a list' in result.output
