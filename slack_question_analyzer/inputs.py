...
def default_data_path(name: str) -> Path:
    """
    Resolve a repo-root data file (taxonomy.json, seed_topics.json).
    The copy in the CURRENT directory wins (it's the user-editable one);
    when it isn't there, fall back to the file shipped next to the package
    (the git-checkout root), so running the CLI or server from another
    directory doesn't silently lose routing buckets or seed topics.
    """
    local = Path(name)
    if local.is_file():
        return local
    shipped = Path(__file__).resolve().parent.parent / name
    return shipped if shipped.is_file() else local
