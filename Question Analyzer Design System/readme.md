...
- **Codebase:** the Flask API server (`api_server.py`) plus the live dashboard
  under `ui_kits/analyzer/` in this folder. The API defines the data contract this
...
> This system IS the shipped dashboard: `ui_kits/analyzer/index.html` is what
> `api_server.py` serves at http://localhost:5000.
...
- **Substitution:** this system uses **[Lucide](https://lucide.dev)**, vendored
  locally at `vendor/lucide.min.js` (react, react-dom, and babel are vendored
  alongside it — the app works offline and behind proxies), as the closest
  readily-available match — same 2px stroke and
...
- **Fonts** are self-hosted: IBM Plex (SIL OFL) woff2 subsets live in
  `vendor/fonts/` and load via `@font-face` rules in `tokens/fonts.css` — no CDN,
  so the dashboard renders correctly offline and behind corporate proxies.
