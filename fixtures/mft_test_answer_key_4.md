...
The two SFTP-timeout questions ("How do I increase the SFTP connection
timeout?" and "Why do SFTP connections time out before large transfers
finish?") now count as ONE recurring topic (2x). Adopted convention,
matching answer key 7: **one answer resolves both = one topic** — everyone
in that group needs "raise the timeout." Expected recurrences: 1.
